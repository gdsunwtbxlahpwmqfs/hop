...
Workflow 由一系列通过 hop 连接的 [action](actions.md) 组成。就像 [pipeline](../07-管道/pipelines.md) 中的 [transform](../07-管道/transforms.md) 一样，每个 action 都是一个小功能单元。多个 action 的组合使 Hop 开发者能够构建强大的数据编排解决方案。

尽管有一些视觉上的相似之处，workflow 和 pipeline 的运行方式截然不同。

- Workflow 执行编排任务。Workflow 中的 action 通常不直接操作数据（尽管您_可以_通过例如 [SQL](../04-动作插件/数据库操作类/sql.md) 更改数据）。
- Workflow 有一个（且只有一个）必填的起始点（[Start](../04-动作插件/工作流控制类/start.md) action），但可以有多个结束 action。
- Workflow 可以
...
Workflow 然后继续执行一个 [pipeline](../04-动作插件/工作流控制类/pipeline.md) action "first-pipeline"。顾名思义，此 action 执行一个 pipeline。
...
** [创建 Workflow](create-workflow.md)
** [运行和调试 Workflow](run-debug-workflow.md)
** [Workflow 运行配置](workflow-run-configurations.md)
