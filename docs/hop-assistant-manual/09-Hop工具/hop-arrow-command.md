...
当 Hop Flight 服务器接收到数据时，它会尝试将指定的路径与 [Data Stream](metadata-types/data-stream/data-stream.md) 名称进行匹配。该 data stream 的类型必须为 "Apache Arrow Flight"。
此时，指定的 [Schema Definition](metadata-types/static-schema-definition.md) 会与接收到的 [Schema](https://arrow.apache.org/cookbook/py/schema) 进行匹配。
数据行被存储在内存中，以便后续被获取。
...
另请参见：[Apache Arrow Flight Data Stream](metadata-types/data-stream/arrow-flight.md)
