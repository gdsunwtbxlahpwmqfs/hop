...
[Workflow executor^](pipeline/transforms/workflow-executor.md) 和 [Pipeline executor^](pipeline/transforms/pipeline-executor.md) 提供了灵活而优雅的方式，可以从现有 pipeline 中运行 workflow 和 pipeline。
...
Pipeline executor transform 默认会逐行将数据发送到子 pipeline。可以通过 `Row grouping` 标签页更改此默认行为。如果向子 pipeline 发送多行数据，请在子 pipeline 中使用 [Get rows from result^](pipeline/transforms/getrowsfromresult.md) transform 来获取这些行。
...
此行集将包含复制到结果中的任何文件名，例如通过 [Text file input^](pipeline/transforms/textfileinput.md) transform 的 `Content` 标签页中的 `Add filenames to result`。
...
除了 workflow 和 pipeline executor transform 之外，[Repeat^](workflow/actions/repeat.md) 和 [End Repeat^](workflow/actions/repeat-end.md) action 允许您从 workflow 构建循环。
