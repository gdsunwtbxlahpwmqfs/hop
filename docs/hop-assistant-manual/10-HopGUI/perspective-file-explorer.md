...
File Explorer perspective 也是你通过 [Git 集成](hop-gui-git.md) 管理项目版本控制的地方。
...
- 打开所选文件：使用右箭头或双击打开所选文件。你也可以直接双击文件来打开它。如果是 workflow 或 pipeline，它将在 [Data Orchestation](perspective-data-orchestration.md) perspective 中打开。在所有其他情况下，文件将在新标签页中打开。
- 创建文件夹：添加一个新文件夹。
...
File Explorer perspective 可通过 [Configuration perspective](perspective-configuration.md) 的 Plugins 标签页进行配置。以下选项可用：
