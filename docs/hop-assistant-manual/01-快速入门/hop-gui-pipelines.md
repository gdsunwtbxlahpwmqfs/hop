...
在[核心概念](getting-started/hop-concepts.md)中，我们介绍了 Pipeline、Transform 和 hop。
让我们回顾一下：
...
现在，向你的 Pipeline 添加一个 [Generate Rows](pipeline/transforms/rowgenerator.md) 和一个 [Add Sequence](pipeline/transforms/addsequence.md) Transform。

![](../assets/images/getting-started/gs-hop-gui-create-pipeline-add-two-transforms.png)

> **💡 提示:** 查看 [Transform 完整列表](pipeline/transforms.md)。
...
查看可用的[运行时配置](pipeline/pipeline-run-configurations/pipeline-run-configurations.md)，以在其他引擎上运行你的 Pipeline。
...
> **💡 提示:** 如需更详细的信息，请查看[运行、预览和调试 Pipeline](pipeline/run-preview-debug-pipeline.md)页面。
