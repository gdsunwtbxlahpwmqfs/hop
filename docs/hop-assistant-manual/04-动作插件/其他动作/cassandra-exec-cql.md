...
| 选项 | 描述 |
|---|---|
| [Cassandra Connection](metadata-types/cassandra/cassandra-connection.md) | 您可以指定要使用的连接名称，可以是固定值或变量表达式。 |
...
可以在此 action 中执行一个或多个查询。查询的输出将被添加到 action 的结果行中。您可以使用 [Get Rows From Result](pipeline/transforms/getrowsfromresult.md) transform 等 plugin 或 [JavaScript](workflow/actions/eval.md) action 来访问这些结果行。
