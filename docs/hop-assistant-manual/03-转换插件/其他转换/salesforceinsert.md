# ![Salesforce Insert transform Icon, role="image-doc-icon"](../../assets/images/transforms/icons/salesforceinsert.svg) Salesforce Insert
...
- Transform name：指定画布上 Salesforce Insert transform 的唯一名称。
您可以自定义名称或保留默认值。
...
- Salesforce ID fieldname：指定包含您插入的记录集（批次）中第一条记录的 Salesforce ID 的字段。
