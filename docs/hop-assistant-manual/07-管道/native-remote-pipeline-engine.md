...
## Hop 远程 Pipeline Engine

远程运行配置在远程 Hop Server 上运行 pipeline。
...
| 如果您不仅要将当前 pipeline 发送到服务器，还要发送引用的 workflow 和 pipeline，请启用此选项。 |  |
...
此选项不仅将父 pipeline 发送到服务器，还会发送所有其他引用的 pipeline 和 workflow。
例如，如果您有映射 pipeline 或引用的 workflow，它们也会被发送到服务器。
所有使用的 pipeline 和 workflow 以及 pipeline 执行配置的 XML 表示将以 ZIP 归档的形式发送到服务器。
服务器接收此归档并在不解压的情况下运行 pipeline。
为使此功能正常运行，Hop 会更改引用和文件名引用。
例如 `{openvar}PROJECT_HOME{closevar}/mapping.hpl` 将被更改为 `{openvar}Internal.Entry.Current.Folder{closevar}/mapping.hpl`。
这意味着它将尝试使用相对于父文件的路径。
