...
- [Neo4j 连接](../06-元数据类型/neo4j-connection.md)：确保指定 `bolt` 协议而不是默认的 `neo4j` 协议
- 大多数 [Neo4j graph output](../03-转换插件/其他转换/neo4j-graphoutput.md)、[Neo4j output](../03-转换插件/Neo4j图数据库类/neo4j-output.md)、[Cypher transform](../03-转换插件/其他转换/neo4j-cypher.md) 和 [action](../04-动作插件/Neo4j操作类/neo4j-cypherscript.md)（除了索引操作，见下文）似乎都能正常工作。如果您发现任何问题，请创建一个 [bug 工单](https://github.com/apache/hop/issues/)。

## 已知问题

Memgraph 的索引语法与 Neo4j 语法不同，这导致 [Neo4j index](../04-动作插件/Neo4j操作类/neo4j-index.md) 和 [Neo4j constraint](../04-动作插件/Neo4j操作类/neo4j-constraint.md) action 失败。
