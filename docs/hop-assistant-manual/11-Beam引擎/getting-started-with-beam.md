...
- [Beam Spark pipeline engine](pipeline/pipeline-run-configurations/beam-spark-pipeline-engine.md)
- [Beam Flink pipeline engine](pipeline/pipeline-run-configurations/beam-flink-pipeline-engine.md)
- [Beam Dataflow pipeline engine](pipeline/pipeline-run-configurations/beam-dataflow-pipeline-engine.md)
** [Dataflow pipeline 模板](pipeline/beam/dataflowPipeline/google-dataflow-pipeline.md)
- [Beam Direct pipeline engine](pipeline/pipeline-run-configurations/beam-direct-pipeline-engine.md)
...
有一些 Beam 特定 transform 只在提供的 Beam pipeline 执行引擎上工作。例如：[Beam Input](pipeline/transforms/beamfileinput.md) 从一个或多个文件读取文本文件数据，或 [Beam BigQuery Output](pipeline/transforms/beambigqueryoutput.md) 将数据写入 BigQuery。
...
- [Memory Group By](pipeline/transforms/memgroupby.md)：此 transform 允许您跨大数据量聚合数据。使用 Beam 引擎时，它使用 `org.apache.beam.sdk.transforms.GroupByKey`。
- [Merge Join](pipeline/transforms/mergejoin.md)：您可以使用此 transform 连接 2 个数据源。主要区别在于在 Beam 引擎中输入数据不需要排序。用于执行此操作的 Beam 类是：`org.apache.beam.sdk.extensions.joinlibrary.Join`。
- [Generate Rows](pipeline/transforms/rowgenerator.md)：此 transform 用于生成（空/静态）数据行。它可以是固定数量，也可以无限生成行。使用 Beam 引擎时，它使用 `org.apache.beam.sdk.io.synthetic.SyntheticBoundedSource` 或 `org.apache.beam.sdk.io.synthetic.SyntheticUnboundedSource`。
...
- [Group By](pipeline/transforms/groupby.md)：请改用 `Memory Group By`
- [Sort Rows](pipeline/transforms/sort.md)

[Denormaliser](pipeline/transforms/rowdenormaliser.md) transform 在 1.1.0 及更高版本的 Apache Beam 中技术上是正确的。即便如此，您需要考虑该 transform 中键值对的聚合（在一般情况下）只在行的子集上发生。这是因为在 Beam pipeline 中，行到达的顺序会丢失，因为它们会被不断重新打乱以最大化并行度。这与"Local" Hop pipeline 引擎的行为不同。

要解决此问题，您可以对整个数据集应用 [Memory Group By](pipeline/transforms/memgroupby.md) transform 来获取您反规范化的每个字段的第一个非空值。这将产生正确的结果。
...
[Beam Input](pipeline/transforms/beamfileinput.md) 和 [Beam Output](pipeline/transforms/beamfileoutput.md) transform 期望您定义正在读取或写入的文件的布局。
