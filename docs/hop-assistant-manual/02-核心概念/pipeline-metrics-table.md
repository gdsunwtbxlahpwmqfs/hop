> **💡 提示:** 此表中的指标可以通过 [Pipeline Log](metadata-types/pipeline-log.md) 在标准 Pipeline 中捕获和处理（例如写入数据库表或 Apache Kafka 主题）

> **⚠️ 警告:** 此表中可用的指标取决于您的 Pipeline 运行配置。
[Local pipeline engine](pipeline/pipeline-run-configurations/native-local-pipeline-engine.md) 始终实时显示这些指标。

| 指标 | 描述 |
|---|---|
| Copy | 这些指标适用的 Transform 副本。详见 [Specify Copies](pipeline/specify-copies.md) |
