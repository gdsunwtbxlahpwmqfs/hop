...
- 将环境特定设置放在一个或多个[环境](projects/projects-environments.md)配置文件中。这允许您在不更改项目的情况下将项目部署到另一个环境（dev/uat/prod），您只需要配置另一组配置文件。
...
请参阅[日志基础](logging/logging-basics.md)、[日志反射](logging/logging-reflection.md)，或考虑将日志记录到 [Neo4j](technology/neo4j/index.md) 图数据库中。最后这种方式允许您在 Neo4j 视图中浏览日志结果。

其他选项包括 [Pipeline Log](metadata-types/pipeline-log.md)、[Pipeline Probe](metadata-types/pipeline-probe.md)、[Workflow Log](metadata-types/workflow-log.md) 和 [Execution Information Location](metadata-types/execution-information-location.md)。查看可用选项，选择适合您项目和团队的日志策略。
...
如果不同的 pipeline 中有重复的逻辑，请考虑使用 [Simple Mapping](pipeline/transforms/simple-mapping.md) 来避免在 pipeline 中一遍又一遍地重复相同的逻辑。

Simple Mapping 是一个从 [Mapping Input](pipeline/transforms/mapping-input.md) 读取数据并写入 [Mapping Output](pipeline/transforms/mapping-output.md) transform 的 pipeline。

您可以使用 [Simple Mapping](pipeline/transforms/simple-mapping.md) transform 在其他 pipeline 中重用这些工作。
...
- 使用[单元测试](pipeline/pipeline-unit-testing.md)测试您的 pipeline。定期运行所有单元测试，验证结果并在需要时采取行动。
