...
# LLM personal assistant (off by default; enable and configure at runtime)
ENV HOP_LLM_ENABLED=false
ENV HOP_LLM_API_URL=""
ENV HOP_LLM_API_KEY=""
ENV HOP_LLM_MODEL=""
...
# ==========================================
# Remove cloud service plugins and dependencies
# to avoid classloader conflicts and reduce image size
# ==========================================
# Remove cloud-related JARs from WEB-INF/lib
RUN rm -f ${CATALINA_HOME}/webapps/ROOT/WEB-INF/lib/google-*.jar
RUN rm -f ${CATALINA_HOME}/webapps/ROOT/WEB-INF/lib/grpc-googleapis-*.jar
RUN rm -f ${CATALINA_HOME}/webapps/ROOT/WEB-INF/lib/proto-google-*.jar
RUN rm -f ${CATALINA_HOME}/webapps/ROOT/WEB-INF/lib/aws-*.jar
RUN rm -f ${CATALINA_HOME}/webapps/ROOT/WEB-INF/lib/azure-*.jar
RUN rm -f ${CATALINA_HOME}/webapps/ROOT/WEB-INF/lib/microsoft-*.jar

# Remove cloud service plugins
RUN rm -rf ${CATALINA_HOME}/plugins/tech/google/
RUN rm -rf ${CATALINA_HOME}/plugins/tech/aws/
RUN rm -rf ${CATALINA_HOME}/plugins/tech/azure/
RUN rm -rf ${CATALINA_HOME}/plugins/databases/googlebigquery/
RUN rm -rf ${CATALINA_HOME}/plugins/databases/aws/
