...
#
# 用法：
#   sudo bash hop-deploy.sh                          # 默认部署（端口 8080）
#   sudo bash hop-deploy.sh --pkg /tmp/hop-offline   # 指定离线包解压目录
#   sudo bash hop-deploy.sh --port 9090              # 自定义端口
#   sudo bash hop-deploy.sh --base /opt/hop          # 自定义安装根目录
#   sudo bash hop-deploy.sh --heap-mode aggressive   # 对齐 Dockerfile -XX:+AggressiveHeap
#   sudo bash hop-deploy.sh --heap 8g                # 自定义堆大小（fixed 模式）
#   sudo bash hop-deploy.sh --uid 1000 --gid 1000    # 对齐 Dockerfile ARG HOP_UID/GID
#   sudo bash hop-deploy.sh --instance hop-bi        # 多实例：tomcat-run-hop-bi/（同机多 Hop Web）
...
HOP_USER="hop"
HOP_GROUP="hop"
HOP_UID="501"        # 对齐 Dockerfile ARG HOP_UID
HOP_GID="501"        # 对齐 Dockerfile ARG HOP_GID
HOP_USER_HOME="/home/hop"   # 对齐 Dockerfile useradd -d /home/hop
TOMCAT_PORT="8080"
SERVICE_NAME="qi-hop-web"
LOG_LEVEL="Basic"    # 对齐 ENV HOP_LOG_LEVEL

# JVM 堆内存策略：aggressive（对齐 Dockerfile -XX:+AggressiveHeap）| fixed（固定大小）
HEAP_MODE="fixed"
HEAP_SIZE="16g"

# 多实例名：空=单实例（tomcat-run/），非空=tomcat-run-${INSTANCE_NAME}/（同机多 Hop Web）
...
if [ -n "$INSTANCE_NAME" ]; then
    ...
    [ "$SERVICE_NAME" = "qi-hop-web" ] && SERVICE_NAME="qi-hop-web-${INSTANCE_NAME}"
else
...
setup_user_and_dirs() {
    ...
    # 对齐 Dockerfile: groupadd -r hop -g ${HOP_GID}
    getent group "$HOP_GROUP" >/dev/null 2>&1 || groupadd -r "$HOP_GROUP" -g "$HOP_GID"
    # 对齐 Dockerfile: useradd -d /home/hop -u ${HOP_UID} -m -s /bin/bash -g hop hop
    if ! id -u "$HOP_USER" >/dev/null 2>&1; then
        useradd -d "$HOP_USER_HOME" -u "$HOP_UID" -m -s /bin/bash -g "$HOP_GROUP" "$HOP_USER"
    fi
...
EOF
    ...
    # 设置权限：实例目录归 hop 用户；CATALINA_HOME 软件目录保持可读
...
print_summary() {
    ...
    info "  sudo bash $0 --instance bi --port 9090"
}
